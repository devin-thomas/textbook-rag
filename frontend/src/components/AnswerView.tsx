    ...
    <article className="answer-article">
      ...
      {answer.status === "insufficient_evidence" ? (
        ...
        <div className="insufficient-state provider-abstention">
          ...
          {answer.evidence.length > 0 && <button className="show-evidence" onClick={onShowEvidence}><BookIcon /> Inspect retrieved evidence ({answer.evidence.length}) <span>⌄</span></button>}
        </div>
      ) : answer.status === "error" ? (
        ...
        <>
          ...
          <button className="show-evidence" onClick={onShowEvidence}><BookIcon /> Show textbook evidence ({answer.evidence.length}) <span>⌄</span></button>
        </>
      )}
    </article>
  );
